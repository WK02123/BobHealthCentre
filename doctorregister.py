...
class Ui_Form(object):
    ...
    def register(self):
        ...
        try:
            ...
            # Get the current number of doctors
            doctors = db.child("doctors").get()
            if doctors.each() is not None:
                num_doctors = len(doctors.each())
            else:
                num_doctors = 0

            # Create the new doctor ID
            new_doctor_id = f"doctor{num_doctors + 1}"
            ...
            # Push user data to Firebase Realtime Database under "doctors" path with the new ID
            db.child("doctors").child(new_doctor_id).set(doctor_data)
