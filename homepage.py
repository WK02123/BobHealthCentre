...
class Ui_Form(object):
    ...
    def set_appointment(self):
        ...
        try:
            # Verify Firebase token from command-line argument (replace with actual token handling)
            decoded_token = auth.verify_id_token(sys.argv[2])  # Verify token from command line
            username = decoded_token.get('name', '')  # Get username from decoded token
            ...
            appointment_data = {
                ...
                "status": "pending",
                "username": username  # Include username in appointment data
            }
